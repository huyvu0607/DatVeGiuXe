...
public async Task<IActionResult> CheckEmailExists(string email, int? userId = null)
{
    ...
    {
        // Log error nếu cần
        return Json(new { exists = false, error = "Lỗi kiểm tra email" });
    }
}
...
public async Task<IActionResult> Edit(int id, EditUserViewModel model)
{
    ...
    // Check if email already exists for other users - SỬA PHẦN NÀY
    if (!string.IsNullOrWhiteSpace(model.Email))
    {
        var emailExists = await _context.Users
            .AnyAsync(u => u.Email.ToLower() == model.Email.ToLower().Trim() && u.Id != model.Id);
    ...
    if (!ModelState.IsValid)
    {
        // QUAN TRỌNG: Giữ lại CurrentPasswordHash khi return view
        model.CurrentPasswordHash = existingUser.PasswordHash;
        return View(model);
    }

    try
    {
        // CẬP NHẬT TỪNG FIELD THAY VÌ TẠO OBJECT MỚI
        existingUser.Name = model.Name?.Trim();
        existingUser.Email = model.Email?.Trim().ToLower();
        existingUser.Role = model.Role;
        
        // Chỉ cập nhật mật khẩu nếu có nhập mật khẩu mới
        if (!string.IsNullOrWhiteSpace(model.Password))
        {
            existingUser.PasswordHash = ComputeSha256Hash(model.Password);
        }
        ...
        if (result > 0)
        {
            TempData["SuccessMessage"] = $"Thông tin người dùng '{existingUser.Name}' đã được cập nhật thành công!";
            TempData["AlertType"] = "success";
        }
        else
        {
    ...
    {
        // Lỗi database specific
        ModelState.AddModelError("", "Lỗi cơ sở dữ liệu. Vui lòng kiểm tra lại thông tin.");
        model.CurrentPasswordHash = existingUser.PasswordHash;
        return View(model);
    }
    catch (Exception ex)
